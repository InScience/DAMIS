...
ObjectMatrix SAMANN::getProjection(){
        ...
        for (int miu = 1; miu < mTrain - 1; miu++)
        {
            for (int niu = miu + 1; niu < mTrain; niu++)
            {
                for (int j = 0; j < nNeurons; j++)
                {
                    tarp = 0.0;
                    for (int k = 0; k < n; k++)
                        tarp = tarp + w1[j].at(k) * Xp.getObjectAt(miu).getItems().at(k);
                    Y_pasl.getObjectAt(miu).UpdateValue(j, 1 / (1 + exp(-1 * tarp)));
                }
                for (int j = 0; j < d; j++)
                {
                    tarp = 0.0;
                    for (int k = 0; k < nNeurons; k++)
                        tarp = tarp + w2[j].at(k) * Y_pasl.getObjectAt(miu).getItems().at(k);
                    Y_is.getObjectAt(miu).UpdateValue(j, 1 / exp(-1 * tarp));
                }
            }
        }
        for (int miu = 1; miu < mTrain - 1; miu++)
        {
            for (int niu = miu + 1; niu < mTrain; niu++)
            {
                ...
                for (int k = 0; k < d; k++)
                {
                    ...
                    tmp = -2 * lambda * ((distXp - distY) / (distXp * distY)) * (Y_is.getObjectAt(miu).getItems().at(k) - Y_is.getObjectAt(niu).getItems().at(k));
                    delta_L.getObjectAt(miu).UpdateValue(niu, tmp);
                    ddelta_L[miu] = delta_L.getObjectAt(miu).getItems().at(niu) * (1 - Y_is.getObjectAt(miu).getItems().at(k)) * Y_is.getObjectAt(miu).getItems().at(k);
                    ddelta_L[niu] = delta_L.getObjectAt(miu).getItems().at(niu) * (1 - Y_is.getObjectAt(niu).getItems().at(k)) * Y_is.getObjectAt(niu).getItems().at(k);
                    ...
                    {
                        if (maxIteration == 1)
                            w2[j][k] = -1 * eta * (ddelta_L[miu] * Y_pasl.getObjectAt(miu).getItems().at(j) - ddelta_L[niu] * Y_pasl.getObjectAt(niu).getItems().at(j));
                        else
                            w2[j][k] = -1 * eta * (ddelta_L[miu] * Y_pasl.getObjectAt(miu).getItems().at(j) - ddelta_L[niu] * Y_pasl.getObjectAt(niu).getItems().at(j)) - w2[k][j];
                    }
                }
                ...
                for (int j = 0; j < nNeurons; j++)
                {
                    ...
                    //for (int i = 0; i < n; i++)
                    //{
                        ddelta_tarp[miu] = delta_tarp[miu] * (1 - Y_pasl.getObjectAt(miu).getItems().at(j));
                        ddelta_tarp[niu] = delta_tarp[niu] * (1 - Y_pasl.getObjectAt(niu).getItems().at(j));
                        if (maxIteration == 1)
                            w1[j][k] = -1 * eta * (ddelta_tarp[miu] * Y_pasl.getObjectAt(miu).getItems().at(j) - ddelta_tarp[niu] * Y_pasl.getObjectAt(niu).getItems().at(j));
                        else
                            w1[j][k] = -1 * eta * (ddelta_tarp[miu] * Y_pasl.getObjectAt(miu).getItems().at(j) - ddelta_tarp[niu] * Y_pasl.getObjectAt(niu).getItems().at(j)) -w2[j][k];
                    //}
                    }
                }
            }            
    ...
    for (int miu = 0; miu < m; miu++)
    {
        for (int j = 0; j < nNeurons; j++)
        {
            tarp = 0.0;
            for (int k = 0; k < n; k++)
                tarp += w1[j].at(k) * X.getObjectAt(miu).getItems().at(k);
            Y_pasl.getObjectAt(miu).UpdateValue(j, (1 / (1 + exp(-1 * tarp))));              
        }
        for (int j = 0; j < d; j++)
        {
            tarp = 0.0;
            for (int k = 0; k < nNeurons; k++)
                tarp += w2[j].at(k) * Y_pasl.getObjectAt(miu).getItems().at(k);
            Y_is.getObjectAt(miu).UpdateValue(j, 1 / (1 + exp(-1 * tarp)));
        }
    }
    Y = Y_is;
    return  Y;
...
double SAMANN::getMax()
{
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++)
            if (max < X.getObjectAt(i).getItems().at(j))
                max = X.getObjectAt(i).getItems().at(j);
    return max; 
}
...
void SAMANN::NormalizeX()
{
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            value = X.getObjectAt(i).getItems().at(j) / max;
            X.getObjectAt(i).UpdateValue(j, value);
        }
...
bool SAMANN::isIdentical(DataObject obj)
{
    ...
    for (int i = 0; i < n; i++)
    {
        k = 0;
        for (int j = 0; j < m; j++)
            if (obj.getItems().at(j) == Xp.getObjectAt(i).getItems().at(j))
                k++;
        if (k == m)
        {
            ats = true;
