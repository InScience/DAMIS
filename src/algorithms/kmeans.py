...
def gauss(n=100, C=[[0, 1], [1, 0]], M=[[0, 0]]):
...
def generateGaussData():
    X1 = gauss(C=[[1, 0.8], [0.8, 1]])
    X2 = gauss(C=[[1, 0.6], [0.6, 1]], M=[[4, 4]])
    X3 = gauss(C=[[1, -0.6], [-0.6, 1]], M=[[-4, -4]])
    X4 = gauss(C=[[1, -0.6], [-0.6, 1]], M=[[-4, 4]])
    X5 = gauss(C=[[1, 0.6], [0.6, 1]], M=[[4, -4]])
    X = [X1, X2, X3, X4, X5]
    return X

def writeData(X, oFile):
...
def getNextFileName():
    global fileIdx
    fileIdx += 1
    return 'kmeans/tmp/o' + str(fileIdx)

def assignRandomClasses(k, iFileName):
    iFile = open(iFileName, 'rb')
    reader = csv.reader(iFile, delimiter=',', quotechar='"')

    oFileName = getNextFileName()
...
def reassignVectors(iFileName, means):
    iFile = open(iFileName, 'rb')
    oFileName = getNextFileName()
    oFile = open(oFileName, 'wb')
    reader = csv.reader(iFile, delimiter=',', quotechar='"')
    ...
    nMoved = 0
    for row in reader:
        cls = closestClass(np.array(map(float, row[0:-1])), means)
        if cls != int(row[-1]):
            row[-1] = cls
            nMoved += 1
        writer.writerow(row)
...
def group(k, iFileName):
    ...
    nMoved = -1
    for it in range(100):
        means = computeMeans(k, iFileName)
        nMoved, oFileName = reassignVectors(iFileName, means)
...
def divideMean(baseMean, clsMinVals, clsMaxVals):
    ''' baseMean - np.array
        clsMinVals - list
        clsMaxVals - list
        returns:
...
def unsupervisedKMeans(maxK, iFileName, oDir):
    ...
    oFileName = assignRandomClasses(2, iFileName)
    bestRatio = None
    bestK = None
    means = None # means of the k groups
    for k in range(2, maxK + 1):
        iFileName = oFileName
        means, oFileName, inter, intra, minVals, maxVals, sig = group(k, iFileName)
        ...
        minusMean, plusMean = divideMean(baseMean, minVals[splitCls], maxVals[splitCls])
        del means[splitCls]
        means.append(minusMean)
        means.append(plusMean)
        isConverged, oFileName = reassignVectors(oFileName, means)
...
if __name__ == "__main__":
    ...
    iFileName = options.filename # e. g. 'kmeans/gaussData.csv'
    oDir = options.dir # e. g. 'kmeans/res/'

    bestK, bestRatio = unsupervisedKMeans(kMax, iFileName, oDir)
