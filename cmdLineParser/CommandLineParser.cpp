...
CommandLineParser::CommandLineParser(int argc_, char * argv_[],bool switches_on_)
{
    argc=argc_;
    argv.resize(argc);
...
std::string CommandLineParser::get_arg(int i)
{
    if (i>=0&&i<argc)
        return argv[i];
...
std::string CommandLineParser::get_arg(std::string s)
{
